                  ...
                  <FormField
                      ...
                      render={() => (
                          <FormItem>
                              ...
                              <ScrollArea className="h-40 w-full rounded-md border p-4">
                                  {availableModels.map((model) => (<Controller key={model} control={editForm.control} name="compatibleModels" render={({ field }) => (<FormItem key={model} className="flex flex-row items-start space-x-3 space-y-0 mb-4"><FormControl><Checkbox checked={field.value?.includes(model)} onCheckedChange={(checked) => { const newValue = checked ? [...field.value, model] : field.value?.filter((value) => value !== model); field.onChange(newValue); }} /></FormControl><FormLabel className="font-normal">{model}</FormLabel></FormItem>)} />))}
                              </ScrollArea>
                              <FormMessage />
                              <Controller
                                  control={editForm.control}
